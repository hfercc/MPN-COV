...
def main():
    ...
    net = ptcv_get_model("isqrtcovresnet50", pretrained=True)
...
def train(**kwargs):
    ...
    for i, (X, y) in enumerate(data_loader):
        ...
        batch_end = time.time()
        if (i + 1) % verbose == 0:
            logging.info('\tBatch %d: (Raw) Loss %.4f, Accuracy: (%.2f, %.2f, %.2f), Time %3.2f' %
                         (i + 1,
    ...
    logging.info('Train: (Raw) Loss %.4f, Accuracy: (%.2f, %.2f, %.2f), Time %3.2f'%
